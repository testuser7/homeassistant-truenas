...
class TrueNASAppStatsSensor(TrueNASEntity, SensorEntity):
    ...
    def __init__(
        ...
        super().__init__(coordinator, entity_description, uid)
        self._has_valid_interface_payload = False
    ...
    def available(self) -> bool:
        ...
        Network sensors are only valid when they resolve to an actual interface,
        and must also satisfy the base availability conditions (coordinator, etc.).
        """
        if self.entity_description.key in (
            "app_stats_network_rx",
            "app_stats_network_tx",
        ):
            return self._has_valid_interface_payload and super().available
